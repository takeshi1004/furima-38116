  ...
  describe '購入情報の保存' do
    ...
    context '保存できない場合' do
      ...
      it 'prefecture_idを選んでいない（idが1）では登録できない' do
        ...
        @order_ship.valid?
        expect(@order_ship.errors.full_messages).to include("都道府県を選んでください")
      end
      ...
      it 'postal_codeが半角のハイフンを含んだ正しい形式でないと登録できない' do
        ...
        @order_ship.valid?
        expect(@order_ship.errors.full_messages).to include("郵便番号にハイフン(-)を含めてください")
      end
      ...
      it 'itemが紐づいていないと登録できない' do
        ...
        @order_ship.valid?
        expect(@order_ship.errors.full_messages).to include("商品を入力してください")
      end
